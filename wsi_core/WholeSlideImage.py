...
class WholeSlideImage(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        # left = [np.min(each, axis=0) for each in contours]
        for each_contour in foreground_contours:
            self.min_ys.append(each_contour[:,0,1].min())
            self.min_xs.append(each_contour[:,0,0].min())
            self.max_ys.append(each_contour[:,0,1].max())
            self.max_xs.append(each_contour[:,0,0].max())
        self.min_y = min(self.min_ys)
        self.min_x = min(self.min_xs)
        self.max_y = max(self.max_ys)
        self.max_x = max(self.max_xs)
        ...
        # self.print_contour_info()
        # cv2.line(img, (min(self.min_xs), min(self.min_ys)), (max(self.max_xs), min(self.min_ys)), (0, 255, 0), thickness=2)
        # cv2.line(img, (min(self.min_xs), min(self.min_ys)), (min(self.min_xs), max(self.max_ys)), (0, 255, 0), thickness=2)
        # cv2.line(img, (max(self.max_xs), min(self.min_ys)), (max(self.max_xs), max(self.max_ys)), (0, 255, 0), thickness=2)
        # cv2.line(img, (min(self.min_xs), max(self.max_ys)), (max(self.max_xs), max(self.max_ys)), (0, 255, 0), thickness=2)
        # cv2.imwrite('test.png', img)
        # exit()
    
    def print_contour_info(self):
        print((min(self.min_xs), min(self.min_ys)), (max(self.max_xs), max(self.max_ys)))
    ...
    def process_contour_fixed_number(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        
        w, h = 170, 170
        results = []
        w_sz = (self.max_x - self.min_x) * 2**(6-patch_level)//w
        h_sz = (self.max_y - self.min_y) * 2**(6-patch_level)//h
        highest_w = (self.max_x - self.min_x) * 2**(6)
        highest_h = (self.max_y - self.min_y) * 2**(6)  
        threshold = (247 + 247 + 247) * w_sz * h_sz
        print(f'threshold: {threshold}')

        for i in tqdm(range(w)):
            for j in range(h):
                img = np.array(self.wsi.read_region((self.min_x + j * w_sz, self.min_y + i * w_sz), patch_level, (w_sz, h_sz)).convert("RGB"))
                if img.sum() < threshold:
                    results.append((self.min_x + j * w_sz, self.min_y + i * w_sz))           

        if len(results)>1:
            asset_dict = {'coords' :          np.array(results)}
            
            attr = {'patch_size' :            np.array([w_sz, h_sz]), # To be considered...
        ...
        save_path_hdf5 = os.path.join(save_path, str(self.name) + '.h5')
        # with h5py.File(save_path_hdf5, 'w') as f:
        #     f['coords'] = asset_dict['coords']
        #     f['sizes'] = asset_dict['sizes']
        ...
        # self.wsi.read_region((self.min_x*2**6,self.min_y*2**6), patch_level, (self.max_x*2**(6-patch_level) - self.min_x*2**(6-patch_level), self.max_y*2**(6-patch_level) - self.min_y*2**(6-patch_level))).save('test.png')
        # exit()
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        if len(results)>1:
            ...
            attr = {'patch_size' :            (patch_size, patch_size), # To be considered...
                    'patch_level' :           patch_level,
                    'downsample':             self.level_downsamples[patch_level],
                    'downsampled_level_dim' : tuple(np.array(self.level_dim[patch_level])),
                    'level_dim':              self.level_dim[patch_level],
